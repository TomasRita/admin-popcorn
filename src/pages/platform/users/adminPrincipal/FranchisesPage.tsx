...
const getAreaData = (darkMode: boolean) => ({
  ...
  datasets: [
    {
      label: "Franquias",
      data: [200, 180, 240, 220, 260, 280, 300, 320, 340, 360, 380, 400],
      tension: 0.4,
      fill: true,
    ...
    },
    {
      ...
      data: [120, 100, 140, 130, 150, 160, 180, 190, 200, 210, 220, 230],
      tension: 0.4,
      fill: true,
      backgroundColor: darkMode ? "rgba(200, 200, 200, 0.2)" : "rgba(30, 64, 175, 0.2)",
...
const FranchisesPage: React.FC = () => {
  ...
  const metrics = [
    { label: "Total Franquias", value: "5 563", icon: <FileText />, diff: "+95%", color: "text-green-500" },
    { label: "Franquias abertas", value: "535", icon: <ShoppingBag />, diff: "+5%", color: "text-green-500" },
    { label: "Pedidos reprovados", value: "145", icon: <XCircle />, diff: "+5%", color: "text-red-500" },
    { label: "Tempo médio de análise", value: "48h", icon: <Clock />, diff: "", color: "" },
  ];
  ...
  return (
    <div className="w-full min-h-screen bg-white dark:bg-gray-800 space-y-8 p-4 sm:p-6">
        ...
        {metrics.map((m) => (
          ...
          >
            <div>
              ...
              <h3 className="text-lg sm:text-xl font-semibold mt-1">{m.value}</h3>
              {m.diff && (
        ...
        <table className="w-full text-left text-sm min-w-[640px]">
          <thead>
            <tr className="text-xs text-gray-500 dark:text-gray-400 uppercase border-b">
              ...
              <th className="py-2 px-2">Nome da Franquia</th>
              ...
              <tr key={f.name}>
                ...
                <td className="py-3 px-2">
                  <div className="flex flex-col">
                    ...
                    <span className="text-gray-400 dark:text-gray-500 text-xs">{f.email}</span>
                  </div>
                </td>
                <td className="py-3 px-2">{f.sector}</td>
                <td className="py-3 px-2">
                  <span className="px-2 py-1 text-xs bg-green-100 dark:bg-green-900 text-green-600 dark:text-green-400 rounded-full">
                    {f.status}
                  </span>
                </td>
                <td className="py-3 px-2 text-gray-500 dark:text-gray-400">{f.date}</td>
                <td className="py-3 px-2 font-semibold">{f.market}</td>
                <td className="py-3 px-2">{f.variation}</td>
                <td className="py-3 px-2 text-blue-600 dark:text-blue-400 text-xs">Ver detalhes</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        ...
        {/* Area Chart */}
        <div className="bg-white dark:bg-gray-900 rounded-2xl p-4 sm:p-6 shadow">
          <h3 className="text-sm font-medium mb-2">Investimento</h3>
